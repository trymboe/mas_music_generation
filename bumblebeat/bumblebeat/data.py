...
from bumblebeat.bumblebeat.utils.data import (
    create_dir_if_not_exists,
    split_range,
    get_bucket_number,
)
import bumblebeat.bumblebeat.vocabulary
...
def get_corpus(
    dataset_name, data_dir, pitch_classes, time_steps_vocab, processing_conf
):
    """
...
class Corpus:
    ...
    def __init__(
        ...
        self.vocab, self.reverse_vocab = bumblebeat.bumblebeat.vocabulary.create_vocab(
            self.n_instruments,
    ...
    def process_dataset(self, dataset, conf):
        ...
        dev_sequences = [
            mm.midi_to_note_sequence(features["midi"]) for features in dataset
        ]
        ...
        dev_sequences = [
            ...
            > mm.steps_per_bar_in_quantized_sequence(s)
        ]
    ...
    def _augment_stretch(self, note_sequences):
        ...
        """
        augmented_slow = [
            mm.sequences_lib.augment_note_sequence(x, 1.01, 1.1, 0, 0)
            for x in note_sequences
        ]
        augmented_fast = [
            mm.sequences_lib.augment_note_sequence(x, 0.9, 0.99, 0, 0)
            for x in note_sequences
        ]
